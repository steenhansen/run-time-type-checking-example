import { isNumeric, BEGIN_SERVER_ERROR } from "./common-2-import";
...
function romanToInt(roman_str) {
  ...
  function processRoman(current_char) {
    if (cur_column === 'm_thousands')
      next_char = doMs(current_char)
    else if (cur_column === 'd_hundreds')
      next_char = doDs(current_char);
    else if (cur_column === 'c_hundreds')
      next_char = doCs(current_char);
    else if (cur_column === 'l_tens')
      next_char = doLs(current_char);
    else if (cur_column === 'x_tens')
      next_char = doXs(current_char);
    else if (cur_column === 'v_ones')
      next_char = doVs(current_char);
    else if (cur_column === 'i_ones')
      next_char = doIs(current_char);
    return next_char
  }
...
function intToRoman(int_num) {
  ...
  if (int_num > 3999) return 'intToRoman, too big';
